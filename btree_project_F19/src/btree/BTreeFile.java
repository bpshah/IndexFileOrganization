...
public class BTreeFile extends IndexFile implements GlobalConst {
...

	{
		...
		if(headerPage.get_rootId().pid == INVALID_PAGE) {
			...
			BTLeafPage newRootPage = new BTLeafPage(headerPage.get_keyType()); 	// Instantiating the BTLeafPage with the key type headerPage.get_keyType()
			PageId newRootPageID = newRootPage.getCurPage();
			newRootPage.setPrevPage(new PageId(INVALID_PAGE));
			newRootPage.setNextPage(new PageId(INVALID_PAGE));
			
			pinPage(newRootPageID);												// Pinning the page with new Page's ID
			newRootPage.insertRecord(key, rid);
			unpinPage(newRootPageID,true);											// Unpinning the page after modifying the page i.e. inserting the record		
		...
		if(newRootEntry != null) {
			...
			unpinPage(newRootIndexPageId,true);
...

	{
		...
		if(currentPage.getType() == NodeType.INDEX) {
			...
			RID fRID = new RID();
			KeyDataEntry tmpKeyEntry = null;
			
			for(tmpEntry = newIndexPage.getFirst(fRID);currentIndexPage.available_space() > newIndexPage.available_space();tmpEntry = newIndexPage.getFirst(fRID)) {
				tmpKeyEntry = tmpEntry;
				currentIndexPage.insertKey(tmpEntry.key, ((IndexData)tmpEntry.data).getData());
				newIndexPage.deleteSortedRecord(fRID);
			}
			
			if(currentIndexPage.available_space() < newIndexPage.available_space()) {
				newIndexPage.insertKey(tmpKeyEntry.key, ((IndexData)upEntry.data).getData());
				currentIndexPage.deleteSortedRecord(fRID);
			}
		...
		return upEntry;
			...
			for(tmpEntry = currentLeafPage.getFirst(delRID);tmpEntry != null;tmpEntry = currentLeafPage.getFirst(delRID)){
			...
			for(tmpEntry = newLeafPage.getFirst(firstRID);currentLeafPage.available_space() >= newLeafPage.available_space();tmpEntry = newLeafPage.getFirst(firstRID)) {
